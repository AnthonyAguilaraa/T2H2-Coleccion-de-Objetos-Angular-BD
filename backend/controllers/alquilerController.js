...
exports.crearAlquiler = async (req, res) => {
  const alquilerData = req.body;
  console.log('Datos recibidos:', alquilerData);

  if (!alquilerData.clienteId || !alquilerData.vehiculoCodigo) {
    return res.status(400).json({ msg: 'Faltan datos: clienteId o vehiculoCodigo' });
  }

  try {
    // Guardar y actualizar el alquiler y vehículo
    const nuevoAlquiler = new Alquiler(alquilerData);
...
exports.finalizarAlquiler = async (req, res) => {
    ...
    // ELIMINAMOS LAS SESIONES/TRANSACCIONES PARA EVITAR ERRORES EN LOCAL
    try {
        // 1. Actualizar Alquiler
        const alquiler = await Alquiler.findByIdAndUpdate(id, {
            estado: 'FINALIZADO',
        ...
        // Si el ID es incorrecto, 'alquiler' será null y la siguiente línea fallaría
        if (!alquiler) {
            return res.status(404).json({ msg: 'Alquiler no encontrado para finalizar' });
        }

        // 2. Liberar Vehículo
        // Ya no necesitamos pasar { session }
        await Vehiculo.findOneAndUpdate(
            { codigo: alquiler.vehiculoCodigo },
            { estado: 'DISPONIBLE' }
        );
        ...
        // Imprimir el error real en la consola del servidor para que sepas qué pasó
        console.error("Error en finalizarAlquiler:", error);
        res.status(500).json({ msg: 'Error finalizando alquiler', error: error.message });
    }
};
// Versión simplificada sin sesiones para eliminarAlquiler
exports.eliminarAlquiler = async (req, res) => {
    ...
    try {
        ...
        // 2. Si el alquiler estaba ACTIVO, liberamos el vehículo
        ...
        res.json({ msg: 'Alquiler eliminado y vehículo liberado' });
